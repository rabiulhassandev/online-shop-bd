...
<header class="sticky top-0 z-50 bg-white shadow-md">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2">
            <div class="flex items-center justify-between text-xs text-gray-600">
                <div class="flex items-center gap-6">
                    <a href="tel:{{ \App\Models\Setting::get('phone') }}" class="flex items-center gap-1 hover:text-amber-500 transition-colors">
                        ...
                        {{ \App\Models\Setting::get('phone', '01700000000') }}
                    </a>
                    <a href="https://wa.me/{{ \App\Models\Setting::get('whatsapp', '8801700000000') }}" class="flex items-center gap-1 hover:text-green-500 transition-colors">
                ...
                <div class="flex items-center gap-4">
                    <span>🚚 বিনামূল্যে ডেলিভারি ঢাকায়</span>
    ...
    <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div class="flex items-center justify-between gap-4">
            ...
            {{-- Search Bar (Desktop) --}}
            <div class="hidden lg:flex items-center gap-2 flex-1 max-w-md mx-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="{{ route('cart.index') }}" class="relative group">
                    ...
                    @if($cartCount > 0)
                        <span id="cart-count-badge" class="absolute -top-1 -right-1 bg-amber-500 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center">{{ $cartCount }}</span>
                    @else
                        <span id="cart-count-badge" class="hidden absolute -top-1 -right-1 bg-amber-500 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center"></span>
                    @endif
                </a>
        ...
        <div class="hidden lg:flex items-center gap-8 mt-4 pt-3 border-t border-gray-100">
            <a href="{{ route('home') }}" class="text-sm font-medium text-gray-700 hover:text-amber-500 transition-colors {{ request()->routeIs('home') ? 'text-amber-500 border-b-2 border-amber-500 pb-3' : '' }}">হোম</a>
            <a href="{{ route('products.index') }}" class="text-sm font-medium text-gray-700 hover:text-amber-500 transition-colors {{ request()->routeIs('products.*') ? 'text-amber-500 border-b-2 border-amber-500 pb-3' : '' }}">সব পণ্য</a>
            <a href="{{ route('home') }}#contact" class="text-sm font-medium text-gray-700 hover:text-amber-500 transition-colors">যোগাযোগ</a>
        </div>
    </nav>

    {{-- Mobile menu --}}
    <div id="mobile-menu" class="hidden lg:hidden border-t border-gray-100 bg-gray-50 py-4">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 space-y-3">
            ...
            <div class="relative">
                <input 
                    type="text" 
                    placeholder="পণ্য খুঁজুন..." 
                    class="w-full px-4 py-2 pl-10 text-sm border border-gray-300 rounded-lg focus:outline-none focus:border-amber-500"
                >
                <svg class="absolute left-3 top-2.5 w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                </svg>
            ...
            {{-- Mobile Nav Links --}}
            <a href="{{ route('home') }}" class="block px-3 py-2 text-sm font-medium text-gray-700 hover:bg-white rounded-md">হোম</a>
            <a href="{{ route('products.index') }}" class="block px-3 py-2 text-sm font-medium text-gray-700 hover:bg-white rounded-md">সব পণ্য</a>
            <a href="{{ route('home') }}#contact" class="block px-3 py-2 text-sm font-medium text-gray-700 hover:bg-white rounded-md">যোগাযোগ</a>
            <a href="tel:{{ \App\Models\Setting::get('phone') }}" class="block px-3 py-2 text-sm font-medium text-amber-500 hover:bg-white rounded-md">কল করুন</a>
        </div>
    </div>
</header>
