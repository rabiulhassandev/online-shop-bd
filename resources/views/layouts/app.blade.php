...
<header class="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
    <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-2 shrink-0">
                @if(\App\Models\Setting::get('site_logo'))
                    <img src="{{ asset('storage/' . \App\Models\Setting::get('site_logo')) }}" alt="কাতুয়া শার্ট" class="h-10">
                @else
                    <span class="text-2xl font-bold tracking-tight text-gray-900">কাতুয়া <span class="text-amber-500">শার্ট</span></span>
                @endif
            </a>

            {{-- Desktop Nav --}}
            <div class="hidden md:flex items-center gap-8">
                <a href="{{ route('home') }}"         class="text-sm font-medium text-gray-700 hover:text-gray-900 transition-colors {{ request()->routeIs('home') ? 'text-amber-500' : '' }}">হোম</a>
                <a href="{{ route('products.index') }}" class="text-sm font-medium text-gray-700 hover:text-gray-900 transition-colors {{ request()->routeIs('products.*') ? 'text-amber-500' : '' }}">সব পণ্য</a>
                <a href="{{ route('home') }}#contact"  class="text-sm font-medium text-gray-700 hover:text-gray-900 transition-colors">যোগাযোগ</a>
            </div>

            {{-- Right side: Cart + mobile toggle --}}
            <div class="flex items-center gap-4">
                {{-- Cart icon --}}
                <a href="{{ route('cart.index') }}" class="relative p-2 rounded-full hover:bg-gray-100 transition-colors" id="cart-btn">
                    ...
                    @if($cartCount > 0)
                        <span id="cart-count-badge" class="absolute -top-1 -right-1 bg-amber-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center font-bold">{{ $cartCount }}</span>
                    @else
                        <span id="cart-count-badge" class="hidden absolute -top-1 -right-1 bg-amber-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center font-bold"></span>
                    @endif
                </a>
        ...
        {{-- Mobile menu --}}
        <div id="mobile-menu" class="hidden md:hidden pb-4 border-t border-gray-100 mt-2 pt-4 space-y-2">
            <a href="{{ route('home') }}"           class="block px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-md">হোম</a>
            <a href="{{ route('products.index') }}" class="block px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-md">সব পণ্য</a>
            <a href="{{ route('home') }}#contact"   class="block px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-md">যোগাযোগ</a>
        </div>
    </nav>
</header>
