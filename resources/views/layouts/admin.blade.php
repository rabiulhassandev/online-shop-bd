...
<div class="flex h-screen overflow-hidden">
    ...
    <aside class="admin-sidebar fixed inset-y-0 left-0 z-30 w-64 bg-gray-900 flex flex-col lg:static lg:translate-x-0"
        ...
        {{-- Navigation --}}
        <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1" style="scrollbar-gutter: stable; scrollbar-width: none; -ms-overflow-style: none;">
            @php
                $navItems = [
                    ...
                    ['dropdown' => 'location', 'label' => 'লোকেশন', 'icon' => 'M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z', 'items' => [
                        ['route' => 'admin.divisions.index', 'label' => 'বিভাগ'],
                        ['route' => 'admin.districts.index', 'label' => 'জেলা'],
                        ['route' => 'admin.upazilas.index', 'label' => 'উপজেলা/থানা'],
                    ]],
                ...
                @if(isset($item['dropdown']))
                    ...
                    <div x-data="{ open: false }" class="mb-1">
                        <button @click="open = !open"
                                class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors text-gray-300 hover:bg-gray-800 hover:text-white">
                        ...
                        <div x-show="open" class="pl-10 pr-2 py-1 space-y-1 border-l border-gray-700 ml-2 mt-1">
                            @foreach($item['items'] as $subItem)
                                @php $isActive = request()->routeIs(rtrim($subItem['route'], '.index') . '*'); @endphp
                                <a href="{{ route($subItem['route']) }}"
                                   class="block rounded-md px-3 py-2 text-sm font-medium transition-colors
                                          {{ $isActive ? 'bg-amber-500 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800' }}">
                                    {{ $subItem['label'] }}
                                </a>
                            @endforeach
                        </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        @if(session('success') || session('error'))
            <div class="fixed top-4 right-4 z-50 flex flex-col gap-2 flash-message">
                @if(session('success'))
                    <div class="bg-green-50 border border-green-200 text-green-800 rounded-lg px-4 py-3 shadow-lg flex items-center gap-2">
                        <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                        {{ session('success') }}
                    </div>
                @endif
                @if(session('error'))
                    <div class="bg-red-50 border border-red-200 text-red-800 rounded-lg px-4 py-3 shadow-lg flex items-center gap-2">
