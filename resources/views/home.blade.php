...
<section x-data="{
...
}" class="relative overflow-hidden bg-gray-900" style="height: 520px;">
    ...
    {{-- Controls --}}
    <button @click="prev()" class="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/20 hover:bg-white/40 rounded-full flex items-center justify-center text-white transition-colors backdrop-blur-sm">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
    </button>
    <button @click="next()" class="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/20 hover:bg-white/40 rounded-full flex items-center justify-center text-white transition-colors backdrop-blur-sm">
    ...
    <div class="absolute bottom-5 left-1/2 -translate-x-1/2 flex gap-2">
        @foreach($sliders as $i => $s)
            <button @click="go({{ $i }})"
                    :class="current === {{ $i }} ? 'bg-amber-400 w-6' : 'bg-white/50 w-2.5'"
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
    <div class="flex items-end justify-between mb-8">
        ...
        <a href="{{ route('products.index') }}" class="text-sm font-medium text-gray-500 hover:text-amber-500 transition-colors hidden sm:block">সব দেখুন →</a>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <div class="flex items-center justify-between">
                ...
                @if($review->product)
                    <a href="{{ route('products.show', $review->product) }}" class="text-xs text-amber-500 hover:underline">{{ Str::limit($review->product->name, 20) }}</a>
                @endif
            </div>
        </div>
        @endforeach
